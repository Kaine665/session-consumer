import { aggregateProjects, aggregateSessions } from "@sc/core";
export async function listProjects(providers, opts) {
...
export async function listSessions(providers, opts) {
    const limit = parseInt(opts.limit || "20", 10);
    const projects = await aggregateProjects(providers);
    for (const project of projects) {
        if (opts.project && !project.name.includes(opts.project))
            continue;
