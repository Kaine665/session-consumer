...
export async function aggregateSessions(
  ...
  for (const provider of available) {
    if (!project.providers.includes(provider.id)) continue;
    try {
...
export async function loadSessionMessages(
  session: Session,
  providers: SessionProvider[],
): Promise<import("../domain/message.js").Message[]> {
  const provider = providers.find((p) => p.id === session.provider);
  if (!provider) throw new Error(`No provider found for ${session.provider}`);
