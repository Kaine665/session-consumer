 ...
 */
export async function searchAllProviders(query, providers) {
    ...
    for (const provider of targetProviders) {
        ...
        for (const msg of messages) {
            if (allResults.length >= query.limit)
                break;
            // Apply filters
            if (query.messageType && msg.type !== query.messageType)
                continue;
            if (query.hasToolUse && msg.toolUses.length === 0)
                continue;
            if (query.model && msg.model !== query.model)
                continue;
            if (query.dateFrom && msg.timestamp < query.dateFrom)
                continue;
            if (query.dateTo && msg.timestamp > query.dateTo)
                continue;
