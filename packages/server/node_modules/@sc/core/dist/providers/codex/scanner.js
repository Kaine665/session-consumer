...
async function buildSessionMeta(filePath, baseDir) {
    let messageCount = 0;
    let firstTs = "";
    let lastTs = "";
    ...
    await parseJsonl(filePath, (entry) => {
        ...
        if (entry.tool_use || entry.toolUse)
            hasToolUse = true;
        ...
        if (entry.project_name && !projectName)
...
function sessionToProjectPath(filePath, baseDir) {
