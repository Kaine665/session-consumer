...
function findAiderFiles(root: string): string[] {
...
async function loadSessions(projectPath: string): Promise<Session[]> {
  ...
  if (!fs.existsSync(aideFile)) return [];
...
function parseAiderMarkdown(md: string): AiderBlock[] {
...
function extractTimestamp(block: string): string {
  const match = block.match(/at\s+(.+)/);
  if (!match) return "";
  try { return new Date(match[1]).toISOString(); }
  catch { return ""; }
}
...
function extractAiderToolUses(content: string): Array<{ name: string; input: Record<string, unknown>; toolUseId: string }> {
  const results: Array<{ name: string; input: Record<string, unknown>; toolUseId: string }> = [];
  const toolRe = /^>>>\s+(.+)/gm;
  let match;
  let idx = 0;
